    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :imgs="skuImageList[show]" />
          <!-- 小图列表 -->
          <ImageList @ShowImage="showimg" />
        </div>
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd
                  changepirce="0"
                  v-for="ite in item.spuSaleAttrValueList || []"
                  :key="ite.id"
                  :class="{ active: ite.isChecked == 1 }"
                  @click="active(item, ite)"
                >
                  {{ ite.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
...
export default {
  ...
  beforeCreate() {
    this.$store.dispatch("getdetailData", this.$route.params.id);
  },
  computed: {
    ...mapGetters(["skuInfo", "spuSaleAttrList", "categoryView"]),
    ...mapState({ detailData: (state) => state.detail.detailData }),
    skuImageList() {
      return this.skuInfo.skuImageList || ["img"];
    },
  },
  mounted() {},
  methods: {
    ...
    input(event) {
      ...
      this.inputs = parseInt(this.inputs);
    },
    add() {
      this.inputs++;
    },
   ...
   async AddCart(){
      ...
      try {
       ...
       await this.$store.dispatch('addCartDate',datas)
       sessionStorage.setItem("datas",JSON.stringify(this.skuInfo))
        this.$router.push({
                     name: 'AddCartSuccess',
                     query: {
                       id: this.$route.params.id,
